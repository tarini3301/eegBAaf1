...
class BrainAgeModel:
    ...
    def generate_counterfactuals(self, features_dict, predicted_age, model_key="ensemble", target_reduction=2.0):
        ...
        for i, fname in enumerate(self.feature_names):
            ...
            # Search over a range of perturbations
            for delta_pct in np.linspace(-0.5, 0.5, 50):
                ...
                perturbed_scaled = self.scaler.transform(perturbed)
                ...
                if model_key == "ensemble":
                    ...
                    wts = wts / wts.sum()
                    new_pred = sum(
                        float(self.models[k].predict(perturbed_scaled)[0]) * w
                        for k, w in zip(individual_keys, wts)
                    )
                else:
                    new_pred = float(self.models[model_key].predict(perturbed_scaled)[0])

                reduction = predicted_age - new_pred
            ...
            if best_reduction > 0.1:
                ...
                counterfactuals.append({
                    ...
                    "predicted_reduction": round(best_reduction, 2),
                    "change_percent": round(change_pct, 1),
                    "direction": "decrease" if best_new_val < original_val else "increase",
                    "suggestion": f"{'Decrease' if best_new_val < original_val else 'Increase'} {self.feature_display_names[fname]} from {original_val:.1f} → {best_new_val:.1f} to reduce brain age by {best_reduction:.1f} years"
                })
    ...
    def compute_tcav_scores(self, feature_values_scaled, model_key="ensemble"):
        ...
        for concept_name, concept_info in concepts.items():
            ...
            perturbed_pos = feature_values_scaled + epsilon * concept_direction.reshape(1, -1)
            perturbed_neg = feature_values_scaled - epsilon * concept_direction.reshape(1, -1)

            if model_key == "ensemble":
                individual_keys = [k for k in MODEL_INFO.keys() if k != "ensemble"]
                wts = np.array([max(0.01, self.scores[k]) for k in individual_keys])
                wts = wts / wts.sum()
                pred_pos = sum(float(self.models[k].predict(perturbed_pos)[0]) * w for k, w in zip(individual_keys, wts))
                pred_neg = sum(float(self.models[k].predict(perturbed_neg)[0]) * w for k, w in zip(individual_keys, wts))
            else:
                pred_pos = float(self.models[model_key].predict(perturbed_pos)[0])
                pred_neg = float(self.models[model_key].predict(perturbed_neg)[0])

            sensitivity = (pred_pos - pred_neg) / (2 * epsilon)
    ...
    def compute_mc_dropout_uncertainty(self, features_dict, n_forward=30):
        ...
        with torch.no_grad():
            for _ in range(n_forward):
                pred = dnn.model(X_t).numpy().flatten()[0]
    ...
    def predict_full_analysis(self, features_dict, chronological_age=None, model_key="ensemble"):
        """
        Complete clinical analysis: Prediction + XAI + Risk + Disease + Uncertainty + Counterfactuals.
        """
        # Base prediction with SHAP + LIME
        ...
        # Phase B: Risk & Disease
        result["risk_category"] = self.classify_risk(result["predicted_age"], chronological_age, result["brain_age_gap"])
        result["disease_risk"] = self.compute_disease_risk(features_dict, result["predicted_age"], chronological_age or result["predicted_age"])
        result["multi_target"] = self.predict_multi_target(features_dict, result["predicted_age"], chronological_age or result["predicted_age"])

        # Phase C: Uncertainty
        result["mc_dropout"] = self.compute_mc_dropout_uncertainty(features_dict)
