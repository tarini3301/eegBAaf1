...
function displayResults(result) {
    ...
    displayRiskCategory(result.risk_category);
    displayMultiTarget(result.multi_target);
    ...
    ordered.forEach(rec => {
        ...
        card.innerHTML = `
            ...
            <div class="rec-value" style="line-height:1.4;">
                Value: ${rec.value}${rec.unit ? ' ' + rec.unit : ''} · Raw Impact: SHAP ${rec.shap_value >= 0 ? '+' : ''}${rec.shap_value.toFixed(2)}y | LIME ${rec.lime_value !== undefined ? (rec.lime_value >= 0 ? '+' : '') + rec.lime_value.toFixed(2) + 'y' : 'N/A'}<br>
                <span style="opacity:0.8;font-size:0.9em">Age-Adjusted: SHAP ${rec.adjusted_shap_value !== undefined ? (rec.adjusted_shap_value >= 0 ? '+' : '') + rec.adjusted_shap_value.toFixed(2) + 'y' : 'N/A'} | LIME ${rec.adjusted_lime_value !== undefined ? (rec.adjusted_lime_value >= 0 ? '+' : '') + rec.adjusted_lime_value.toFixed(2) + 'y' : 'N/A'}</span>
            </div>
            <ul class="rec-tips">
